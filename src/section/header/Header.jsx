...
const Header = () => {
    ...
    React.useEffect(() => {
        setActiveLink(location.hash || '#'); // 設定目前的 active link
        const path = location.hash || '#';  // 使用 location.hash 取代 pathname

        const handleScroll = () => {
            if (path === '#' || path === '') {
                window.scrollTo({ top: 0, behavior: 'smooth' }); // 滾動到頂部
            } else {
                const targetId = path.substring(1); // 去掉 '#' 符號
                const targetElement = document.getElementById(targetId);
                if (targetElement) {
                    targetElement.scrollIntoView({ behavior: 'smooth' }); // 平滑滾動到目標區塊
                }
            }
        };

        // 延遲滾動以確保頁面內容已經渲染完畢
        setTimeout(handleScroll, 50); // 可根據需要調整延遲時間
    }, [location.hash]);
    ...
    const handleShow = () => {
        setShow(true);
        setTimeout(() => {
            const path = window.location.hash || '#';  // 使用 location.hash，預設為 '#'
            if (modalRef.current) {
        ...
        <>
                ...
                <Container>
                    ...
                    <Modal show={show} onHide={handleClose} ref={modalRef}>
                        ...
                        <Modal.Body className='d-lg-none'>
                            <Nav className='main-navbar' activeKey={activeLink} onSelect={handleSelect} ref={navRef}>
                                <Nav.Link onClick={handleClose} as={Link} to="/" className={activeLink === '/' ? 'active' : ''}>吉屋租賃</Nav.Link>
                                <Nav.Link onClick={handleClose} as={Link} to="#intro" className={activeLink === '#intro' ? 'active' : ''}>房東服務</Nav.Link>
                                <Nav.Link onClick={handleClose} as={Link} to="#services" className={activeLink === '#services' ? 'active' : ''}>專業經營</Nav.Link>
                                <Nav.Link onClick={handleClose} as={Link} to="#featured" className={activeLink === '#featured' ? 'active' : ''}>品質保證</Nav.Link>
                                <Nav.Link onClick={handleClose} as={Link} to="#location" className={activeLink === '#location' ? 'active' : ''}>服務成果</Nav.Link>
                                <Nav.Link onClick={handleClose} as={Link} to="#contact" className={activeLink === '#contact' ? 'active' : ''}>聯絡諮詢</Nav.Link>
                            </Nav>
                        </Modal.Body>
                    </Modal>
                </Container>
            ...
            <Navbar className='p-0'>
                <Nav className='aside-navbar' activeKey={activeLink} onSelect={handleSelect} ref={navRef}>
                    ...
                    <Nav.Link as={Link} to="#intro" className={activeLink === '#intro' ? 'active' : ''}>
                        <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    ...
                    <Nav.Link as={Link} to="#services" className={activeLink === '#services' ? 'active' : ''}>
                    ...
                    <Nav.Link as={Link} to="#featured" className={activeLink === '#featured' ? 'active' : ''}>
                        <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    ...
                    <Nav.Link as={Link} to="#location" className={activeLink === '#location' ? 'active' : ''}>
